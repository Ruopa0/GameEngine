    ...
    fn default() -> Self {
        ...
        engine.register_type_with_name::<Vec3>("Vec3")
            ...
            .register_get_set(
                "x",
                |v: &mut Vec3| v.x,
                |v: &mut Vec3, x: f32| v.x = x,
            )
            .register_get_set(
                "y",
                |v: &mut Vec3| v.y,
                |v: &mut Vec3, y: f32| v.y = y,
            )
            .register_get_set(
                "z",
                |v: &mut Vec3| v.z,
                |v: &mut Vec3, z: f32| v.z = z,
            )
...
impl Plugin for ScriptingPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<ScriptCache>()
            ...
            .add_systems(Update, execute_scripts.run_if(in_state(crate::editor::EngineState::Play)));
    }
}
...
fn execute_scripts(
    ...
    for (script, mut transform) in q_scripts.iter_mut() {
        if script.path.is_empty() { continue; }
        ...
        if let Some(ast) = script_cache.asts.get(&script.path) {
            ...
            match script_cache.engine.eval_ast_with_scope::<()>(&mut scope, ast) {
                Ok(_) => {
