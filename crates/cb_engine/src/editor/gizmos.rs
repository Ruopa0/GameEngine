...
fn draw_gizmos(
    ...
    q_selected: Query<
        (
            Entity,
            &GlobalTransform,
            Option<&super::serialization::EditorLock>,
        ),
        With<Selected>,
    >,
) {
        ...
        match *gizmo_mode {
            GizmoMode::Translate => {
                gizmos.arrow(
                    origin,
                    origin + Vec3::X * 2.0,
                    color(Vec3::X, Color::srgb(1.0, 0.0, 0.0)),
                );
                gizmos.arrow(
                    origin,
                    origin + Vec3::Y * 2.0,
                    color(Vec3::Y, Color::srgb(0.0, 1.0, 0.0)),
                );
                gizmos.arrow(
                    origin,
                    origin + Vec3::Z * 2.0,
                    color(Vec3::Z, Color::srgb(0.0, 0.0, 1.0)),
                );
            }
            GizmoMode::Rotate => {
                gizmos.circle(
                    Isometry3d::new(origin, Quat::from_rotation_arc(Vec3::Z, Vec3::X)),
                    2.0,
                    color(Vec3::X, Color::srgb(1.0, 0.0, 0.0)),
                );
                gizmos.circle(
                    Isometry3d::new(origin, Quat::from_rotation_arc(Vec3::Z, Vec3::Y)),
                    2.0,
                    color(Vec3::Y, Color::srgb(0.0, 1.0, 0.0)),
                );
                gizmos.circle(
            ...
            GizmoMode::Scale => {
                ...
                gizmos.sphere(
                    Isometry3d::from_translation(origin + Vec3::X * 2.0),
                    0.2,
                    color(Vec3::X, Color::srgb(1.0, 0.0, 0.0)),
                );
                gizmos.sphere(
                    Isometry3d::from_translation(origin + Vec3::Y * 2.0),
                    0.2,
                    color(Vec3::Y, Color::srgb(0.0, 1.0, 0.0)),
                );
                gizmos.sphere(
                    Isometry3d::from_translation(origin + Vec3::Z * 2.0),
                    0.2,
                    color(Vec3::Z, Color::srgb(0.0, 0.0, 1.0)),
                );
            }
...
fn draw_remote_presence(
    ...
    q_locked_objects: Query<
        (&GlobalTransform, &super::serialization::EditorLock),
        Without<Selected>,
    >,
) {
    // 1. Draw remote teammate camera frustums and view cones
    for (gt, cam) in q_remote_cameras.iter() {
        if cam.user_id == session.client_id {
            continue;
        }
    ...
    if active_gizmo.dragging_axis.is_none() {
        ...
        if viewport_state.is_hovered {
            ...
            for (entity, _, global_transform, _) in q_selected.iter() {
                ...
                for axis in axes {
                    let (t_ray, t_axis) =
                        closest_points_line_line(ray_origin, ray_dir, origin, axis);
...
fn apply_gizmo_drag(
    ...
    let Some((drag_entity, axis, drag_start_t)) = current_axis else {
        return;
    };
    ...
    match *gizmo_mode {
        GizmoMode::Translate => {
            if active_gizmo.dragging_axis.is_some()
                ...
                if delta.abs() > 0.0001 {
                    for (_, mut transform, _, net_id, lock_opt) in q_selected.iter_mut() {
                        ...
                        if let Some(nid) = net_id {
                            action_requests.write(
                                super::serialization::EditorActionRequest::MoveObject {
                                    id: nid.0,
                                    transform: *transform,
        ...
        GizmoMode::Rotate => {
            ...
            for (_, mut transform, _, net_id, lock_opt) in q_selected.iter_mut() {
                ...
                if axis == Vec3::X {
                    transform.rotate_x(movement * sensitivity);
                }
                if axis == Vec3::Y {
                    transform.rotate_y(movement * sensitivity);
                }
                if axis == Vec3::Z {
                    transform.rotate_z(movement * sensitivity);
                }
...
fn handle_hotkeys(keys: Res<ButtonInput<KeyCode>>, mut gizmo_mode: ResMut<GizmoMode>) {
